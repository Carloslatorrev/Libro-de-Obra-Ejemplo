...
namespace LODapp.ViewModels
{
    ...
    class RechazarDocumentoViewModel : ViewModelBase
    {
        ...
        public async Task RechazarDoc()
        {
            ...
            if (connection)
            {
                docAnotInet = await InternetAnotacionServices.RechazarDoc(IdDocAnotacion.ToString());
            }
            else
            {
                docAnotInet = null;
